...
class compensationInput extends Controller
{
    public function conpensationView(){
        return view('input.conpensation', [
            'taxs' => Compensation::all()

        ]);
    }
    public function conpensationInput(Request $request){
        ...
        Compensation::create($tax);
        Expense::create($tax);
        Comment::create($tax);
        Document::create($tax);
        
        return redirect('/Compensation/input')->with('tax', 'data added successfully');
    ...
    public function download(){
        ...
        foreach ($taxs as $tax) {
            ...
            $sheet->setCellValue('D' . $row, $tax['company']);
